...
extending the theme, and global anti-patterns — **`agent_harness_rails/skills/writing-css-tailwind/references/patterns.md`**. This section stays **template- and helper-focused**.
...
**Theme extension** (`bg-brand-500` vs repeated `bg-[#…]`) and **dark strategy** (`class` vs `media`) — **`agent_harness_rails/skills/writing-css-tailwind/references/patterns.md`**.

---

## Stimulus and Hotwire (Turbo)

Canonical patterns — Stimulus (targets, actions, values), **`turbo_frame_tag`**, stream templates (**`*.turbo_stream.erb`**), morphing-friendly markup, **`data-turbo-permanent`**, and accidental-SPA anti-patterns — live in **`agent_harness_rails/skills/writing-hotwire/references/patterns.md`**. Model broadcast APIs (**`broadcasts_refreshes_to`**, **`turbo_stream_from`**, granular **`broadcast_*`**) stay in **`agent_harness_rails/skills/writing-models/references/patterns.md`** § Turbo Broadcasts.
...
Mailer classes, **`deliver_later`**, multipart templates, **`Mailer.with`**, **`ActionMailer::Preview`**, and mailer specs: **`agent_harness_rails/skills/writing-mailers/references/patterns.md`**. Rule: **`agent_harness_rails/rules/mailers.mdc`**.
