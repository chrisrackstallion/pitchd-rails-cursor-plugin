...
description: >-
  ...
  rules/testing.mdc. Discover related specs, audit each test against the Five
  ...
  older suite to current plugin conventions. Not for writing new specs for
...
- The user has a heavy system-spec suite and wants it slimmed against the budget in `writing-tests/references/system-specs.md`.
...
- A new plugin version (or a fresh review) reveals layering violations across an existing suite.
...
- **Read `../writing-tests/SKILL.md` and `../writing-tests/references/system-specs.md`** if not already in context. The Five Gates and Budget are the rubric you will apply.
- **Skim `../../rules/testing.mdc`** for the ownership table and anti-pattern table.
...
- **REWRITE** — keep the intent but rewrite to match plugin patterns (e.g. `rack_test` instead of Selenium, inline setup instead of nested `let`).
...
5. **Run RuboCop on touched files.** `bin/rubocop spec/system/articles_spec.rb spec/requests/articles_spec.rb …` — fix all offences before continuing per `../running-rubocop/SKILL.md`.
...
**REWRITE** — keep the file and intent but bring it to plugin patterns:
...
- **Run RuboCop.** `bin/rubocop` — zero offences per `../running-rubocop/SKILL.md`.
...
- **Do not change production code.** If a test reveals a bug or an awkward model API, name it in the report and stop. Production refactors run in a separate session per `../implementing-pitchd-rails/SKILL.md`.
