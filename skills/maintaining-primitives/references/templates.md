...
provenance. Structure and ownership rules: the pitchd-rails plugin's
`rules/primitives.mdc` (canonical — this file only records app-specific
...
the plugin, and it would change what an agent proposes.** Target under 50 lines.
...
in the pitchd-rails plugin (compass + rules) — only what is true of THIS app
...
- [Deltas only: constraints specific to this capability, not derivable from
  plugin rules, not stated in compilation.md. Link, don't repeat.]
