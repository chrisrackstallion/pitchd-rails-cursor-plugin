...
Canonical patterns — Stimulus (targets, actions, values), **`turbo_frame_tag`**, stream templates (**`*.turbo_stream.erb`**), morphing-friendly markup, **`data-turbo-permanent`**, and accidental-SPA anti-patterns — live in **`../../writing-hotwire/references/patterns.md`**. Model broadcast APIs (**`broadcasts_refreshes_to`**, **`turbo_stream_from`**, granular **`broadcast_*`**) stay in **`../writing-models/references/patterns.md`** § Turbo Broadcasts.
