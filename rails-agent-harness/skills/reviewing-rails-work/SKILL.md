...
description: >-
  ...
  rails-reviewer subagent (see `rails-agent-harness/agents/rails-reviewer.md`).
---
...
(`writing-*`, `rails-agent-harness/rules/*.mdc`) for **correct usage**. Do not duplicate the
...
`rails-agent-harness/rules/services.mdc`" — not "you might want to consider". The harness rules exist
...
Read **`rails-agent-harness/skills/rails-omakase-compass/SKILL.md`**. Use it for:
...
Runs when the app has a **`docs/primitives/`** tree (see `rails-agent-harness/rules/primitives.mdc`).
...
- **Eval coverage** — plan phase: every touched clause gets a spec home in the
  ...
  **already-existing specs** (lazy backfill, prior capture, earlier
  close-outs) and `unproven` rows (`rails-agent-harness/rules/primitives.mdc`) are legitimately
...
finding (one home: a concern or the owning model, `rails-agent-harness/rules/models.mdc`);
...
in a concern or on the owning model (`rails-agent-harness/rules/models.mdc`).
...
`writing-*` skills, and `rails-agent-harness/rules/*.mdc` — apply even when the application currently does
otherwise. An established application pattern is not a justification; it may be exactly
the debt worth naming. State violations directly: "This violates `rails-agent-harness/rules/services.mdc`:
...
- Would the implementor have to add a method, object, or abstraction that
  fails the "earns its place" test (`rails-agent-harness/rules/models.mdc`, `rails-agent-harness/rules/services.mdc`)?
...
- [confidence: X.X] `[file or area]`: [Direct statement — untraceable behaviour, compilation/Shape contradiction, missing eval row, supersession hygiene, provenance conflict.] — `rails-agent-harness/rules/primitives.mdc`
...
subagents](https://cursor.com/docs/subagents)) at **`rails-agent-harness/agents/rails-reviewer.md`**
...
- **RuboCop (zero offences, no disables):** `rails-agent-harness/skills/running-rubocop/SKILL.md`, `rails-agent-harness/rules/rubocop.mdc`
- **Compass (why / whether):** `rails-agent-harness/skills/rails-omakase-compass/SKILL.md`
- **Planning:** `rails-agent-harness/skills/writing-rails-plans/SKILL.md` (plans should be reviewed with this skill via **`rails-reviewer`**, Phase `plan`)
- **Full plan execution (orchestrator):** `rails-agent-harness/skills/executing-rails-plan/SKILL.md` — loops implementor → this reviewer until Approved, then user sign-off
- **Implementation (execute a task):** `rails-agent-harness/skills/implementing-rails-task/SKILL.md` — **`rails-implementor`** for isolated task execution with compass + tactics
