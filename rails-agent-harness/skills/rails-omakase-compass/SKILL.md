...
description: >-
  ...
  and rails-agent-harness/rules/*.mdc. Not for Stimulus/Turbo syntax, plan templates, or file layout.
---
...
`skills/writing-*` and `rails-agent-harness/rules/*.mdc`; this skill is the **compass**: defaults,
...
are a smell (see `writing-services`, `rails-agent-harness/rules/services.mdc`).
...
- **`rails-agent-harness/rules/*.mdc` and `writing-*` skills** win on **specifics** (this repo’s HOW).
