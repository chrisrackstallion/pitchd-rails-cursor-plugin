...
Server-rendered HTML first. Controllers own **HTTP responses**; templates and Stimulus own **markup and behaviour**. Model callbacks own **broadcasts** when updates should reach other tabs or users — see **`rails-agent-harness/skills/writing-models/references/patterns.md`** § Turbo Broadcasts for `broadcasts_refreshes`, `broadcast_append_later_to`, etc.
...
**File location, importmap pins, and bundling** — not duplicated here: **`rails-agent-harness/skills/writing-javascript/references/patterns.md`**.
...
See **`rails-agent-harness/skills/writing-views/references/patterns.md`** § Links and Buttons for extended examples — behaviour matches this document.
