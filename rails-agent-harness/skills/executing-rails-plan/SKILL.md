...
- A **written plan** exists (see `rails-agent-harness/skills/writing-rails-plans/SKILL.md`) and the user wants it **executed**.
...
whole tree (`rails-agent-harness/rules/primitives.mdc` reading contract). The capability doc
...
- Instruction: follow **`rails-agent-harness/skills/implementing-rails-task/SKILL.md`**, no `git commit` unless the user explicitly overrode that elsewhere.

Use the prompt template in **`rails-agent-harness/agents/rails-implementor.md`** as the shape of the dispatch.
...
Instruction: follow **`rails-agent-harness/skills/reviewing-rails-work/SKILL.md`** and return the standard **harness review** report.
...
The implementor never commits (`rails-agent-harness/skills/implementing-rails-task/SKILL.md`) — version
...
plus **`bin/rubocop` zero offences** (`rails-agent-harness/skills/implementing-rails-task/SKILL.md`).
...
- **The intent changed** ("it should do X instead"): "fixed" contradicts a
  clause. Amend the clauses (supersede / add, per `rails-agent-harness/rules/primitives.mdc`)
...
- Instruction: scope the change tightly — **do not** refactor surrounding code
  outside the revision; follow `rails-agent-harness/skills/implementing-rails-task/SKILL.md` and
  `rails-agent-harness/rules/rubocop.mdc`; no `git commit`.
...
- **BLOCKED / NEEDS_CONTEXT** from implementor → stop, present the blocker to
  ...
  (`rails-agent-harness/rules/primitives.mdc`); note the reversal in chat, not provenance.
...
- **Focus (name it in the prompt):** cross-task coherence the per-task loops
  ...
  entity** (one home: a concern or the owning model, `rails-agent-harness/rules/models.mdc`),
  duplicated logic across tasks, naming drift between tasks, and duplicate
  test coverage across layers (`rails-agent-harness/rules/testing.mdc`).
...
- **Plans:** `rails-agent-harness/skills/writing-rails-plans/SKILL.md`
- **Implement:** `rails-agent-harness/skills/implementing-rails-task/SKILL.md` — **`rails-implementor`**
- **Review:** `rails-agent-harness/skills/reviewing-rails-work/SKILL.md` — **`rails-reviewer`**
- **Primitives:** `rails-agent-harness/skills/maintaining-primitives/SKILL.md`, `rails-agent-harness/rules/primitives.mdc` — **`rails-primitives-maintainer`** for structural passes
- **Subagent definitions:** `rails-agent-harness/agents/rails-implementor.md`, `rails-agent-harness/agents/rails-reviewer.md`, `rails-agent-harness/agents/rails-primitives-maintainer.md`
