...
- This harness's `rules/` and `writing-*` skills do not already answer it.
- The user explicitly asks you to consult the official Rails guides.

**Prefer first:** relevant `writing-*` skills and `rails-agent-harness/rules/*.mdc` — they are the
...
| `writing-*`, `rails-agent-harness/rules/*.mdc` | **How** we ship — primary project contract. |
