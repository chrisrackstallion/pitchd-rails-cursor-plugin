...
Prefer **`t()`** for static copy; use **lazy lookup** (`t(".title")`) where it matches the template path. Helpers and shared partials use **absolute keys**. Interpolation, pluralization, and YAML layout: **`rails-agent-harness/skills/writing-i18n/references/patterns.md`**.
...
extending the theme, and global anti-patterns — **`rails-agent-harness/skills/writing-css-tailwind/references/patterns.md`**. This section stays **template- and helper-focused**.
...
**Theme extension** (`bg-brand-500` vs repeated `bg-[#…]`) and **dark strategy** (`class` vs `media`) — **`rails-agent-harness/skills/writing-css-tailwind/references/patterns.md`**.

---

## Stimulus and Hotwire (Turbo)

Canonical patterns — Stimulus (targets, actions, values), **`turbo_frame_tag`**, stream templates (**`*.turbo_stream.erb`**), morphing-friendly markup, **`data-turbo-permanent`**, and accidental-SPA anti-patterns — live in **`rails-agent-harness/skills/writing-hotwire/references/patterns.md`**. Model broadcast APIs (**`broadcasts_refreshes_to`**, **`turbo_stream_from`**, granular **`broadcast_*`**) stay in **`rails-agent-harness/skills/writing-models/references/patterns.md`** § Turbo Broadcasts.

```erb
<%# Stimulus lives in app/javascript/controllers/ — no inline scripts %>
<div data-controller="clipboard" class="flex items-center gap-2">
  <input data-clipboard-target="source" value="<%= @article.share_url %>" class="..." readonly>
...
Mailer classes, **`deliver_later`**, multipart templates, **`Mailer.with`**, **`ActionMailer::Preview`**, and mailer specs: **`rails-agent-harness/skills/writing-mailers/references/patterns.md`**. Rule: **`rails-agent-harness/rules/mailers.mdc`**.
