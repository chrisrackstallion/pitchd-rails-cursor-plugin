...
Markup placement (`class_names`, partials, helpers, forms) lives in **`rails-agent-harness/skills/writing-views/references/patterns.md`**. This document covers **build pipeline**, **global CSS**, **config**, and **boundaries** with Hotwire and JS.
...
- Dozens of `.btn-primary` / `.card` classes built from `@apply` — you re-created Bootstrap. Use partials and `btn_classes`-style helpers instead (`rails-agent-harness/skills/writing-views/references/patterns.md`).
...
- **Responsive:** `sm:`, `md:`, `lg:` live on the same elements as content — behaviour is visible in the template (`rails-agent-harness/skills/writing-views/references/patterns.md`).
...
Field-level error styling and flash markup are **views** concerns — **`rails-agent-harness/skills/writing-views/references/patterns.md`** § Forms. This skill does not duplicate every form example; it enforces **one** theme and **no** duplicate design system in JS.
...
- Pair **`data-*`** hooks with minimal JS — **`rails-agent-harness/skills/writing-hotwire/references/patterns.md`**.
...
- Semantic HTML first; utilities decorate structure. **`rails-agent-harness/skills/writing-views/references/patterns.md`** covers structure; this skill reminds that **contrast** and **visible focus** are non-negotiable.
...
| Need | See |
|------|-----|
| ERB, partials, `class_names`, helpers | `rails-agent-harness/skills/writing-views/references/patterns.md` |
| Turbo, Stimulus behaviour | `rails-agent-harness/skills/writing-hotwire/references/patterns.md` |
| importmap, bundlers, `application.js` | `rails-agent-harness/skills/writing-javascript/references/patterns.md` |
