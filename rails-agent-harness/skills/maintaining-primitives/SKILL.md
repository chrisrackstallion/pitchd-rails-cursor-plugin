...
Structure and boundaries: **`rails-agent-harness/rules/primitives.mdc`** (applies automatically to
paths under `docs/primitives/`). Skeletons: **`references/templates.md`**.
One-time adoption (tree scaffold + `compilation.md` interview):
**`rails-agent-harness/skills/bootstrapping-primitives/SKILL.md`**.
...
7. Add the `index.md` line, `status: built` — the feature exists in
   ...
   blocking the status (`rails-agent-harness/rules/primitives.mdc`).
...
- Size tripwires from `rails-agent-harness/rules/primitives.mdc`: >~10 active clauses (split the
...
**`rails-primitives-maintainer`** (`rails-agent-harness/agents/rails-primitives-maintainer.md`)
with operation + paths/goals; this skill stays canonical.

## Related

- **`rails-agent-harness/rules/primitives.mdc`** — structure, ownership, size limits.
