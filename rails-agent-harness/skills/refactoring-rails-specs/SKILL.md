...
description: >-
  ...
  rails-agent-harness/rules/testing.mdc. Discover related specs, audit each test against the Five
...
- The user has a heavy system-spec suite and wants it slimmed against the budget in `rails-agent-harness/skills/writing-tests/references/system-specs.md`.
...
- **Read `rails-agent-harness/skills/writing-tests/SKILL.md` and `rails-agent-harness/skills/writing-tests/references/system-specs.md`** if not already in context. The Five Gates and Budget are the rubric you will apply.
- **Skim `rails-agent-harness/rules/testing.mdc`** for the ownership table and anti-pattern table.
...
5. **Run RuboCop on touched files.** `bin/rubocop spec/system/articles_spec.rb spec/requests/articles_spec.rb …` — fix all offences before continuing per `rails-agent-harness/skills/running-rubocop/SKILL.md`.
...
- **Run RuboCop.** `bin/rubocop` — zero offences per `rails-agent-harness/skills/running-rubocop/SKILL.md`.
...
- **Do not change production code.** If a test reveals a bug or an awkward model API, name it in the report and stop. Production refactors run in a separate session per `rails-agent-harness/skills/implementing-rails-task/SKILL.md`.
...
This skill can be delegated to the **`rails-implementor`** subagent at `rails-agent-harness/agents/rails-implementor.md` when the work is scoped to a single resource and the parent wants to keep the main context clean. The implementor agent has the writing-tests skill and tooling to execute the verdicts and verify; the parent passes the input spec list, baseline expectations, and a pointer to this skill in the task prompt.
...
- **Test-writing conventions:** `rails-agent-harness/skills/writing-tests/SKILL.md` (`references/system-specs.md`, `references/request-specs.md`, `references/model-specs.md`, `references/support-specs.md`, `references/factory-patterns.md`)
- **Testing rule:** `rails-agent-harness/rules/testing.mdc` (ownership table, anti-patterns, budget, Five Gates)
- **RuboCop:** `rails-agent-harness/skills/running-rubocop/SKILL.md` — run after every refactor batch
- **Implementor subagent:** `rails-agent-harness/agents/rails-implementor.md` — optional delegation
- **Reviewer skill / subagent:** `rails-agent-harness/skills/reviewing-rails-work/SKILL.md` — run after a large refactor to confirm harness fit
- **Compass:** `rails-agent-harness/skills/rails-omakase-compass/SKILL.md` — read only if the refactor surfaces a philosophy question (HTML vs API, server vs client truth); usually not needed
