...
- **Mailers** — `article_mailer.published.subject` (works with
  **`default_i18n_subject`** in mailers) — see **`rails-agent-harness/skills/writing-mailers/references/patterns.md`**.
...
- **Policies / `rescue_from`** — e.g. `I18n.t("pundit.article_policy.update?")` — see **`rails-agent-harness/skills/writing-policies/references/patterns.md`**
...
Detail: **`rails-agent-harness/skills/writing-mailers/references/patterns.md`** § I18n and subjects.
...
See **`rails-agent-harness/rules/testing.mdc`** — behaviour has one home; do not re-test every
