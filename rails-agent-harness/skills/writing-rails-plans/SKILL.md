...
`rails-agent-harness/skills/writing-tests/SKILL.md` / `rails-agent-harness/rules/testing.mdc` (RSpec + FactoryBot).
...
1. **Read `rails-agent-harness/skills/rails-omakase-compass/SKILL.md` first** — before the file
...
**`rails-agent-harness/rules/primitives.mdc`** and `maintaining-primitives`). If the tree does not
...
- **REST is the vocabulary:** New behavior is usually **new resources** or
  **standard CRUD**, not RPC routes — see `rails-agent-harness/rules/routes.mdc`. If the plan adds
...
- **Domain logic lives on the model** (and concerns, form objects, jobs — not
  ...
  that only forwards to Active Record — see `rails-agent-harness/rules/services.mdc`.
- **Authorization is explicit:** Tasks must name **policy** changes (`authorize`,
  `policy_scope`) — see `rails-agent-harness/rules/policies.mdc`.
- **UI is server-rendered first:** Turbo Drive → Frames → Streams as complexity
  grows — see `rails-agent-harness/rules/hotwire.mdc`, `rails-agent-harness/skills/writing-hotwire/SKILL.md`,
  `rails-agent-harness/rules/views.mdc`, `rails-agent-harness/rules/javascript.mdc`.
- **Tests are behaviour-first:** **System spec** for user-visible flows when
  possible; request / model / policy specs only where
  `rails-agent-harness/skills/writing-tests/SKILL.md` says — **one home per behaviour**, no duplicate
  coverage across layers (`rails-agent-harness/rules/testing.mdc`).
...
4. **Schema changes follow expand → migrate → contract across PRs.** The
   ...
   `rails-agent-harness/rules/migrations.mdc`.
...
`rails-agent-harness/rules/migrations.mdc`, controller tasks before `rails-agent-harness/rules/controllers.mdc`, spec
tasks before `rails-agent-harness/skills/writing-tests/SKILL.md` — and so on for every row the
...
| Area | Read |
|------|------|
| Architecture, omakase fit, boundaries ("whether" before "how") | `rails-agent-harness/skills/rails-omakase-compass/SKILL.md` |
| Models, domain verbs, state-as-records | `rails-agent-harness/rules/models.mdc`, `rails-agent-harness/skills/writing-models/SKILL.md` |
| Routes, shallow nesting, REST | `rails-agent-harness/rules/routes.mdc`, `rails-agent-harness/skills/writing-routes/SKILL.md` |
| Controllers, params, Hotwire response order | `rails-agent-harness/rules/controllers.mdc`, `rails-agent-harness/skills/writing-controllers/SKILL.md` |
| Pundit | `rails-agent-harness/rules/policies.mdc`, `rails-agent-harness/skills/writing-policies/SKILL.md` |
| No service layer / where logic goes | `rails-agent-harness/rules/services.mdc` |
| Tests (RSpec, FactoryBot, spec types) | `rails-agent-harness/rules/testing.mdc`, `rails-agent-harness/skills/writing-tests/SKILL.md` |
| Hotwire, Stimulus | `rails-agent-harness/rules/hotwire.mdc`, `rails-agent-harness/skills/writing-hotwire/SKILL.md` |
| Views, partials, components discipline | `rails-agent-harness/rules/views.mdc`, `rails-agent-harness/skills/writing-views/SKILL.md` |
| Tailwind / CSS | `rails-agent-harness/rules/css-tailwind.mdc`, `rails-agent-harness/skills/writing-css-tailwind/SKILL.md` |
| JavaScript | `rails-agent-harness/rules/javascript.mdc`, `rails-agent-harness/skills/writing-javascript/SKILL.md` |
| I18n | `rails-agent-harness/rules/i18n.mdc`, `rails-agent-harness/skills/writing-i18n/SKILL.md` |
| Mailers | `rails-agent-harness/rules/mailers.mdc`, `rails-agent-harness/skills/writing-mailers/SKILL.md` |
| Background jobs | `rails-agent-harness/rules/jobs.mdc`, `rails-agent-harness/skills/writing-jobs/SKILL.md` |
| Migrations | `rails-agent-harness/rules/migrations.mdc`, `rails-agent-harness/skills/writing-migrations/SKILL.md` |
| Naming (classes, methods, columns, routes, specs) | `rails-agent-harness/rules/naming.mdc`, `rails-agent-harness/skills/writing-naming-conventions/SKILL.md` |
| Linting | `rails-agent-harness/rules/rubocop.mdc`, `rails-agent-harness/skills/running-rubocop/SKILL.md` |

### Supplementary reference (optional)

When **`rails-omakase-compass`**, this skill's philosophy section, and the scoped **`rails-agent-harness/rules/*.mdc`** / **`writing-*`** rows above still leave a **Rails best-practice** gap, two sources are available — use the one that fits:

- **`rails-agent-harness/skills/referencing-unofficial-37signals-guide/SKILL.md`** — for 37signals / Fizzy-derived patterns and philosophy not spelled out in the harness (README TOC → raw `.md`).
- **`rails-agent-harness/skills/referencing-rails-guides/SKILL.md`** — for **authoritative Rails API and feature docs** (GitHub API index → specific guide `.md`).
...
6. **Views / partials** — `app/views/...` per `rails-agent-harness/skills/writing-views/SKILL.md`.
7. **Hotwire / Stimulus** — only if the UX needs frames, streams, or client
   behavior (`rails-agent-harness/skills/writing-hotwire/SKILL.md`).
8. **I18n** — user-facing copy (`rails-agent-harness/rules/i18n.mdc`) when adding or changing strings.
9. **Mailer** — only if the spec requires email (`rails-agent-harness/rules/mailers.mdc`).
10. **Jobs** — only if the spec requires background processing (`rails-agent-harness/rules/jobs.mdc`).
11. **Tests** — spec type per `rails-agent-harness/skills/writing-tests/SKILL.md`: system spec for
...
> follow **`rails-agent-harness/skills/writing-tests/SKILL.md`** and **`rails-agent-harness/rules/testing.mdc`**. Prefer
...
**Conventions:** `rails-agent-harness/skills/rails-omakase-compass/SKILL.md` (shape), `rails-agent-harness/rules/models.mdc`,
`rails-agent-harness/rules/routes.mdc`, `rails-agent-harness/rules/controllers.mdc`, `rails-agent-harness/rules/policies.mdc`, `rails-agent-harness/rules/services.mdc`,
`rails-agent-harness/rules/testing.mdc`, `rails-agent-harness/rules/hotwire.mdc` (and UI rules as needed — see table above)
...
- [ ] **Step 1: Write the failing test** (system / request / model / policy per `rails-agent-harness/skills/writing-tests/SKILL.md`)
...
- Introducing **service objects**, **repository layers**, or **RPC routes** without
  matching justification and alignment with `rails-agent-harness/rules/services.mdc` / `rails-agent-harness/rules/routes.mdc`
...
- **`app/services` wrappers** that only call Active Record — use model verbs,
  form objects, or jobs (`rails-agent-harness/rules/services.mdc`).
- **Custom member routes** for state that belongs in **`update`** or a **small
  ...
  (`rails-agent-harness/rules/routes.mdc`).
- **Duplicate system + request + model** coverage for the **same** behaviour
  (`rails-agent-harness/rules/testing.mdc`).
- **Standalone `not_to` assertions** that only prove an element is absent — every
  ...
  side-effect assertion, not a primary one (`rails-agent-harness/rules/testing.mdc`).
- **Turbo Streams** before **redirect** / **frame** solutions when the UX allows
  (`rails-agent-harness/rules/controllers.mdc`, `rails-agent-harness/skills/writing-hotwire/SKILL.md`).
...
- **The same method or behaviour defined on more than one entity** across tasks —
  ...
  (`rails-agent-harness/rules/models.mdc`). Per-entity copies drift the moment one changes; tasks
...
4. **Duplication scan:** Read the plan as a whole — not task by task — and
   ...
   (`rails-agent-harness/rules/models.mdc`) — and the other tasks reference it.
...
(`rails-agent-harness/agents/rails-reviewer.md` — [Cursor subagents](https://cursor.com/docs/subagents)).
**Canonical workflow and report format:** `rails-agent-harness/skills/reviewing-rails-work/SKILL.md`.
...
**`rails-agent-harness/skills/executing-rails-plan/SKILL.md`**: delegate each task to
