...
description: >-
  ...
  writing-* skills and rails-agent-harness/rules/*.mdc for tactics. Use when executing a single task
  ...
  rails-agent-harness/agents/rails-implementor.md).
---
...
and `rails-agent-harness/rules/*.mdc`. Implement what the task asks — no extra framework, no
...
adds a new action, write the action using model logic per `rails-agent-harness/rules/services.mdc`,
...
| `writing-*` + `rails-agent-harness/rules/*.mdc` | **How** to write routes, models, controllers, Hotwire, tests, etc., for this repo. |
...
| `writing-tests` | Tests: DHH/37signals philosophy (system backbone, real objects, behaviour over mocks) plus `rails-agent-harness/rules/testing.mdc`. |
| `running-rubocop` | **Lint gate:** `bin/rubocop` **zero offences** before DONE/review — fix code only, no inline or config disables — see `rails-agent-harness/rules/rubocop.mdc`. Not a substitute for compass or tests. |
| `maintaining-primitives` | **Primitives tree only** — capability docs, `compilation.md`, provenance under `docs/primitives/` per `rails-agent-harness/rules/primitives.mdc`. Implementation **reads** intent/shape excerpts pasted into the task prompt but **never writes** to the tree — the planner and execution close-out own those write points. If the task is code, stay in this skill. |

**Conflict rule (same as reviewing-rails-work):**

- **Tactics** (`writing-*`, `rails-agent-harness/rules/*.mdc`) win on **specific HOW**.
...
Read **`rails-agent-harness/skills/rails-omakase-compass/SKILL.md`** when the task involves:
...
migration means `writing-migrations` + `rails-agent-harness/rules/migrations.mdc` are open first;
touching a spec means `writing-tests` + `rails-agent-harness/rules/testing.mdc`; and so on. "I
...
**`rails-agent-harness/agents/rails-implementor.md`** runs this workflow in an
...
- **RuboCop:** `rails-agent-harness/skills/running-rubocop/SKILL.md`, `rails-agent-harness/rules/rubocop.mdc`
- **Compass:** `rails-agent-harness/skills/rails-omakase-compass/SKILL.md`
- **Review after implementation:** `rails-agent-harness/skills/reviewing-rails-work/SKILL.md` — `rails-reviewer` (includes surroundings pass for pre-existing code in touched files)
