...
description: >-
  ...
  shape) plus writing-* skills and rails-agent-harness/rules/*.mdc; may use
...
**Canonical workflow:** Read **`rails-agent-harness/skills/implementing-rails-task/SKILL.md`**
...
- **Tactics:** Always align implementation with **`skills/rails-omakase-compass`**
  for shape, then **`writing-*`** and **`rails-agent-harness/rules/*.mdc`** for specifics — never
...
logic per `rails-agent-harness/rules/services.mdc`, not by calling into an existing service object.
...
**`rails-agent-harness/rules/*.mdc`** still leave a **Rails best-practice** gap, two supplementary
sources are available — use the one that fits the gap:

- **`rails-agent-harness/skills/referencing-unofficial-37signals-guide/SKILL.md`** — for 37signals /
  Fizzy-derived patterns, philosophy, and tradeoffs not spelled out in the harness.
- **`rails-agent-harness/skills/referencing-rails-guides/SKILL.md`** — for **authoritative Rails API
...
3. **Read order** — Per the skill: **`rails-agent-harness/skills/rails-omakase-compass/SKILL.md`**
   when boundaries apply, then only the **`writing-*`** skills and
   **`rails-agent-harness/rules/*.mdc`** files relevant to the task (include **`rails-agent-harness/rules/rubocop.mdc`**
...
`rails-agent-harness/skills/implementing-rails-task/SKILL.md`.
...
2. Write tests per writing-tests / rails-agent-harness/rules/testing.mdc (TDD if task says so).
3. Verify: tests **and**, if the app uses RuboCop, **`skills/running-rubocop`** — **`bin/rubocop` exit 0, zero offences** before DONE/review; fix in code only (no disable comments or cop suppressions in YAML). **BLOCKED** if truly unfixable — see implementing-rails-task.
