...
Read **`rails-agent-harness/skills/maintaining-primitives/SKILL.md`** from the workspace root and
...
Apply **`rails-agent-harness/rules/primitives.mdc`** for any edits under `docs/primitives/**/*.md`.
...
- **Boundaries:** **`compilation.md`** is human-owned — propose amendments in
  ...
  and the execution revision loop own it otherwise (`rails-agent-harness/rules/primitives.mdc`).
